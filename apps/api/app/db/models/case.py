...
class Case(Base, TimestampMixin):
    ...
    trigger_type: Mapped[TriggerType] = mapped_column(
        portable_enum(TriggerType, "trigger_type")
    )
...
class ClosedCase(Base, TimestampMixin):
    ...
    Canonical closed-case graph data lives in TigerGraph (per project decision).
    """
    ...
    outcome: Mapped[PriorOutcome] = mapped_column(
        portable_enum(PriorOutcome, "prior_outcome")
    )
    pattern: Mapped[Pattern] = mapped_column(portable_enum(Pattern, "pattern"))
    first_fraud_txn_id: Mapped[str | None] = mapped_column(String(32), nullable=True)
    txn_ids: Mapped[list[str]] = mapped_column(JSON, default=list)
    n_txns: Mapped[int] = mapped_column(Integer, default=0)
    exposure_usd: Mapped[float] = mapped_column(Float, default=0.0)
    connected_card_ids: Mapped[list[str]] = mapped_column(JSON, default=list)
